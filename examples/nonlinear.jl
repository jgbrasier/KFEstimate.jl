...
dyn = NonLinearDynamicModel(f, Q)
obs = NonLinearObservationModel(h)

ekf = ExtendedKalmanFilter(dyn, obs);
# run simulation
time_step = 0.0:dt:10
x0 = [0.0, 1.0]
action_sequence = [[0.0; 0.0] for t in time_step]
sim_states, sim_measurements = run_simulation(ekf, R_gt, x0, action_sequence)
# run kalman filter
P0 = Matrix{Float64}(I, 2, 2)
s0 = State(x0, P0) # initial state belief
filtered_states = run_filter(ekf, R_gt, s0, action_sequence, sim_measurements)
